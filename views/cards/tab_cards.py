...
from .presentation import DOCUMENT_EVENT_COLUMN, STATUS_COLUMN, TABLE_GRID_STYLESHEET, TABLE_ITEM_ALIGNMENT, card_row_colors, cards_row_height
...
class TabCards(QWidget):
    ...
    def __init__(self, parent=None, viewmodel: TabCardsViewModel = None):
        ...
        # Віджет тримає поточні знімки даних для трьох таблиць,
        # щоб швидко перевідображати їх без зайвих звернень до Qt-моделей.
        ...
        # Верхня таблиця є головною: вибір у ній визначає вміст двох нижніх таблиць.
        ...
        # Нижні таблиці синхронізуються з вибраною карткою і показують її історію,
        # тому їх не наповнюємо напряму до першого коректного вибору рядка.
    ...
    def _configure_table_width(self, table: QTableWidget, width_weights: list[int]):
        # Залишаємо ручне керування ширинами, щоб зберігати контроль над пропорціями
        # і тримати фільтри точно під відповідними колонками.
    ...
    def _create_cards_filter_bar(self, parent_layout: QVBoxLayout):
        ...
        # Окреме поле під кожну колонку дозволяє фільтрувати таблицю комбіновано,
        # не будуючи для цього окрему проксі-модель Qt.
    ...
    def _apply_table_widths(self, table: QTableWidget, width_weights: list[int]):
        ...
        if table is self.cards_table:
            # Фільтри повинні залишатися точно вирівняними з колонками після ресайзу.
            self._sync_cards_filter_widths()
    ...
    def _reload_all_tables(self, card_id: int | None = None):
        ...
        # а потім відновлюємо вибір, щоб автоматично підтягнулися залежні записи.
        self._load_cards()
    ...
    def _refresh_cards_table(self):
        ...
        for row_index, card in enumerate(self._cards):
            ...
            # У UserRole зберігаємо первинний ключ запису, щоб подальші дії
            # не залежали від тексту в комірці, сортування чи прихованих рядків.
            for column_index, value in enumerate(visible_values):
                item = QTableWidgetItem(value)
                item.setTextAlignment(TABLE_ITEM_ALIGNMENT)
        ...
        # Після повного перемальовування заново відновлюємо зв'язок з нижніми таблицями,
        # бо вибір користувача міг змінитися через сортування або оновлення списку.
        self._on_card_selection_changed()

    def _cards_row_height(self, value: str) -> int:
        return cards_row_height(value)
    ...
    def _update_cards_row_heights(self):
        ...
        for row_index, card in enumerate(self._cards):
            document_value = card.visible_values()[DOCUMENT_EVENT_COLUMN]
            status_value = card.visible_values()[STATUS_COLUMN]
            row_height = max(
                self._cards_row_height(document_value),
                self._cards_row_height(status_value),
            )
            if row_index == selected_row:
                # Для активного рядка враховуємо ще й багаторядкову примітку.
    ...
    def _apply_cards_filter(self):
        ...
        for row_index in range(self.cards_table.rowCount()):
            ...
            for column_index, filter_value in enumerate(filter_values):
                ...
                # Фільтрація працює як перетин умов по всіх колонках:
                # рядок залишається видимим лише якщо проходить кожен непорожній фільтр.
    ...
    def _selected_card(self):
        ...
        # Повертаємо повну доменну модель картки, а не лише id,
        # щоб нижчі обробники могли працювати з її станом без повторного читання таблиці.
    ...
    def _on_card_selection_changed(self):
        ...
        if selected_card is None:
            # Якщо вибір зник після фільтрації чи оновлення, нижні таблиці треба очистити,
            # інакше в них залишиться історія вже неактуальної картки.
            self._clear_admission_table()
            self._clear_access_table()
            self._update_admission_buttons_state()
            self._update_access_buttons_state()
        ...
        # Обидві нижні таблиці завжди відносяться до однієї й тієї самої поточної картки.
    ...
    def resizeEvent(self, event):
        ...
        # На зміну розміру реагуємо вручну, бо ширини колонок прив'язані до ваг,
        # а не до стандартних режимів розтягування Qt.
